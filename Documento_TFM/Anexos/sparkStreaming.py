...
# current_date para fecha actual cuando este en produccion
...
def getBlackShapes(sparkContext):
 if ("BlackShapes" not in globals()):
  ...
  blk_shp = blk_shp.withColumn("lat_min", 
              (format_number(blk_shp.lat, 3) - 0.005).cast(FloatType())).\
    withColumn("lat_max", 
              (format_number(blk_shp.lat, 3) + 0.005).cast(FloatType())).\
    withColumn("long_min", 
              (format_number(blk_shp.long, 3) - 0.005).cast(FloatType())).\
    withColumn("long_max", 
              (format_number(blk_shp.long, 3) + 0.005).cast(FloatType()))
 ...
 def process(time, rdd):
  ...
  try:
   ...
   if (not rdd.isEmpty()):
    ...
    df = df.withColumn('observationDate', from_unixtime(
     unix_timestamp('observationTime', format_1))).\
    ...
    dataToSend = joinData.select("Type", "altitude",
            "coordinates_lat",
            "coordinates_long", "date",
            "observationTime", "observationDate",
            "dateSend", "serverDate",
            "heading", "location",
            "stream.sensorId", "serverTime",
            "speed", "speedmetric",
            "temp", "id_user", "name")
    ...
    schema4udf = StructType([StructField("addrs_name", StringType()),
           StructField(
            "max_speed", IntegerType())
          ])
    reference_to_dict_udf = udf(reference_to_dict, schema4udf)

    # Obtenemos la georeferenciacion
    dataToSend = dataToSend.withColumn("data_osm",
     reference_to_dict_udf(struct(
     [dataToSend[x] for x in ['coordinates_lat', 'coordinates_long',
            'Type', 'speed']])))
    dataToSend = dataToSend.select("id", "Type", "altitude",
            "coordinates_lat", "coordinates_long",
            "date", "observationTime",
            "observationDate", "dateSend",
            "serverDate", "serverTime",
            "heading", "location",
            "stream.sensorId", "speed",
            "speedmetric", "temp",
            "id_user", "name",
         col("data_osm.addrs_name").alias(
          "addrs_name"),
         col("data_osm.max_speed").alias("max_speed"))
    ...
    nearBlkShp = nearBlkShp.select(col("id"), col("Address"),
            col("Province"), col("Country"),
            col("numAccident"), col(
             "lat").alias("blk_point_lat"),
            col("long").alias(
             "blk_point_long"),
    ...
    finalNearBlkShp = minD4.alias('mins').join(nearBlkShp.alias('dataBlkShp'),
     ...
     col("dataBlkShp.blk_point_lat").alias('blk_point_lat'),
     col("dataBlkShp.blk_point_long").alias('blk_point_long'),
    ...
    dataToSend = dataToSend.select(col("data.Type").alias("Type"),
            col("data.altitude").alias(
             "altitude"),
            col("data.observationTime").alias(
             "observationTime"),
            col("data.dateSend").alias(
             "dateSend"),
            col("data.serverTime").alias(
             "serverTime"),
            col("data.heading").alias(
             "heading"),
            col("data.location").alias(
             "location"),
            col("data.sensorId").alias(
             "sensorId"),
            col("data.speed").alias(
             "speed"),
            col("data.speedmetric").alias(
             "speedmetric"),
            col("data.temp").alias("temp"),
            col("data.id_user").alias(
             "id_user"),
            col("data.name").alias("user"),
            col("data.addrs_name").alias(
             "actual_address"),
            col("data.max_speed").alias(
             "max_speed"),
    ...
    dataToSend.select(to_json(
         struct([dataToSend[x] for x in dataToSend.columns])).
           alias("value")).write.format("kafka").
